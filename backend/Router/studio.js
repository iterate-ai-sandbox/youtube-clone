...
Studio.post('/deletevideo/:videoId', async (req, res) => {
 try {
  ...
  await videodata.updateOne({'VideoData._id': videoId}, {$pull: {VideoData: {_id: videoId}}})

  await TrendingData.deleteOne({videoid: videoId})

  await userData.updateMany({'likedVideos.likedVideoID': videoId}, {$pull: {likedVideos: {likedVideoID: videoId}}})

  await userData.updateMany({'watchLater.savedVideoID': videoId}, {$pull: {watchLater: {savedVideoID: videoId}}})

  await userData.updateMany({'Playlists.playlist_videos.videoID': videoId}, {$pull: {'Playlists.$.playlist_videos': {videoID: videoId}}})
...
Studio.post('/savevideoeditdetails/:videoId', async (req, res) => {
 try {
  ...
  await userData.updateMany(
   ...
   {
    arrayFilters: [{'playlist.playlist_videos.videoID': videoId}, {'video.videoID': videoId}]
   }
  )
...
Studio.get('/getallcomments/:email', async (req, res) => {
 try {
  ...
  const videoid = video.VideoData.flatMap(data => data.comments.map(item => item.videoid))
...
Studio.get('/checklikecomment/:commentId/:email', async (req, res) => {
 try {
  ...
  const checkLikeComment = user.likedComments.find(item => item.comment_ID.toString() === commentId.toString())
...
Studio.get('/getvideocommentsbyid/:videoId', async (req, res) => {
 try {
  ...
  const findComments = video.VideoData.filter(item => item._id.toString() === videoId.toString())
