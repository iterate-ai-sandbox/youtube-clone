...
Channel.get('/getchannelid/:email', async (req, res) => {
 try {
  ...
  if (user?.hasChannel) {
   ...
   res.status(200).json({channelID, subscribers, channelDescription, links})
  } else {
   return res.status(404).json({
...
Channel.post('/savechannel', async (req, res) => {
 try {
  const {email, ChannelName, ChannelAbout, fblink, instalink, twitterlink, websitelink, profileURL, currentDate} = req.body
...
Channel.post('/subscribe/:channelID/:email/:email2', async (req, res) => {
 try {
  ...
  const existingChannelIndex = user.subscribedChannels.findIndex(channel => channel.channelID.toString() === channelID.toString())
...
Channel.get('/checksubscription/:channelID/:email', async (req, res) => {
 try {
  ...
  const existingChannelID = user.subscribedChannels.find(channel => channel.channelID.toString() === channelID)
...
Channel.post('/savefeaturedchannel/:email', async (req, res) => {
 try {
  ...
  const channelExists = featuredChannelData.some(channel => channel.channelID === data.channelID)
...
Channel.post('/savecustomization/:email', async (req, res) => {
 try {
  ...
  await videodata.updateMany(
   ...
   {
    arrayFilters: [{'video.comments.user_email': email}, {'comment.user_email': email}]
   }
  )
