...
Videos.post('/publish', async (req, res) => {
 try {
  const {videoTitle, videoDescription, tags, videoLink, thumbnailLink, video_duration, email, publishDate, Visibility} = req.body
...
Videos.get('/getvideos', async (req, res) => {
 try {
  ...
  const videoURLs = videos.flatMap(video => video.VideoData.map(data => data.videoURL))
  const videoData = videos.flatMap(video => video)
  const thumbnailURLs = videos.flatMap(video => video.VideoData.map(data => data.thumbnailURL))
  const titles = videos.flatMap(video => video.VideoData.map(data => data.Title))
  const Uploader = videos.flatMap(video => video.VideoData.map(data => data.uploader))
  const Duration = videos.flatMap(video => video.VideoData.map(data => data.videoLength))
  const Profile = videos.flatMap(video => video.VideoData.map(data => data.ChannelProfile))
  const videoID = videos.flatMap(video => video.VideoData.map(data => data.id))
  const comments = videos.flatMap(video => video.VideoData.map(data => data.comments))
  const views = videos.flatMap(video => video.VideoData.map(data => data.views))
  const uploadDate = videos.flatMap(video => video.VideoData.map(data => data.uploaded_date))
  const Likes = videos.flatMap(video => video.VideoData.map(data => data.likes))
  const Visibility = videos.flatMap(video => video.VideoData.map(data => data.visibility))
...
Videos.post('/updateview/:id', async (req, res) => {
 try {
  ...
  const videoIndex = video.VideoData.findIndex(data => data._id.toString() === id)
...
Videos.post('/watchlater/:id/:email/:email2', async (req, res) => {
 try {
  ...
  const existingSavedVideo = user.watchLater.find(savedVideo => savedVideo.savedVideoID === WatchLater._id.toString())

  if (existingSavedVideo) {
   user.watchLater = user.watchLater.filter(savedVideo => savedVideo.savedVideoID !== WatchLater._id.toString())
...
Videos.get('/checkwatchlater/:id/:email', async (req, res) => {
 try {
  ...
  const userIndex = user.watchLater.findIndex(data => data.savedVideoID.toString() === id.toString())
...
Videos.get('/checktrending/:videoID/:email', async (req, res) => {
 try {
  ...
  if (timeDiffHours < 56 && Views >= 50 && !trendingVideo) {
 ...
 try {
  ...
  const filteredVideos = video.reduce((accumulator, element) => {
   const filteredVideoData = element.VideoData.filter(item => {
    const includesTitle = item.Title.toLowerCase().includes(data.toLowerCase())
    const includesTags = item.Tags.toLowerCase().includes(data.toLowerCase())
  ...
  const filteredChannels = users.filter(userData => userData.channelData.some(channel => channel.channelName.toLowerCase().includes(data.toLowerCase())))
...
Videos.post('/addplaylist/:email', async (req, res) => {
 try {
  const email = req.params.email
  const {playlist_name, playlist_privacy, playlist_date, playlist_owner, thumbnail, title, videoID, description, videolength, video_uploader, video_date, video_views, videoprivacy} = req.body
...
Videos.post('/addvideotoplaylist/:email', async (req, res) => {
 try {
  const email = req.params.email
  const {Id, thumbnail, title, videoID, description, videolength, video_uploader, video_date, video_views, videoprivacy} = req.body
  ...
  const playlistToUpdate = user.Playlists.find(playlist => playlist._id.toString() === Id.toString())

  if (!playlistToUpdate) {
   return res.status(404).json({error: 'Playlist not found'})
  }

  const isVideoExists = playlistToUpdate.playlist_videos.some(video => video.videoID === videoID)
...
Videos.get('/getvideodataplaylist/:email/:videoId', async (req, res) => {
 try {
  ...
  const playlistsWithVideo = user.Playlists.filter(playlist => playlist.playlist_videos.some(video => video.videoID === videoId))
...
Videos.get('/getplaylists/:playlistID', async (req, res) => {
 try {
  ...
  const myPlaylists = user.Playlists.find(item => item._id.toString() === playlistID.toString())
...
Videos.post('/saveplaylist/:playlistID', async (req, res) => {
 try {
  ...
  const playlistIndex = user.Playlists.findIndex(item => item._id.toString() === playlistID.toString())
...
Videos.post('/deleteplaylist/:playlistID', async (req, res) => {
 try {
  ...
  user.Playlists = user.Playlists.filter(item => item._id.toString() !== playlistID.toString())
  await user.save()
 } catch (error) {
  res.status(500).json({error: error.message})
 }
...
Videos.post('/addotherplaylist/:playlistID/:email', async (req, res) => {
 try {
  ...
  const checkPlaylistAvailability = user.savedPlaylists.find(item => item.playlistID.toString() === playlistID.toString())
  ...
  if (!checkPlaylistAvailability) {
   ...
   user.savedPlaylists = user.savedPlaylists.filter(item => item.playlistID !== playlistID.toString())
   await user.save()
...
Videos.get('/getsavedplaylist/:email', async (req, res) => {
 try {
  ...
  const savedPlaylistIDs = user.savedPlaylists.map(playlist => playlist.playlistID)

  const matchingPlaylists = []

  // Iterate through all users and their playlists
  AllUsers.forEach(currentUser => {
   const playlistsForUser = currentUser.Playlists.filter(playlist => savedPlaylistIDs.includes(playlist._id.toString()))
