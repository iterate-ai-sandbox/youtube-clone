...
Comments.post('/comments/:id', async (req, res) => {
 try {
  ...
  const vid = video.find(vidData => vidData.VideoData.some(data => data._id.toString() === id))

  if (!vid) {
   return res.status(404).json({error: 'Video not found'})
  }

  const videoIndex = vid.VideoData.findIndex(data => data._id.toString() === id)
...
Comments.post('/likecomment/:videoId/:commentId/:email', async (req, res) => {
 try {
  ...
  const likedCommentIndex = user.likedComments.findIndex(likedComment => likedComment.comment_ID === commentId)
  ...
  if (likedCommentIndex === -1) {
   ...
   await user.save()
   await video.save()
   return res.status(200).json({message: 'Comment liked successfully', likes: comments})
  } else {
   user.likedComments.splice(likedCommentIndex, 1)
   comment.likes -= 1
   await user.save()
   await video.save()
   return res.status(200).json({message: 'Comment disliked successfully', likes: comments})
  }
 } catch (error) {
...
Comments.post('/heartcomment/:videoId/:commentID', async (req, res) => {
 try {
  ...
  const comments = video.VideoData[videoIndex].comments
  const findComment = comments.find(item => item._id.toString() === commentID.toString())
...
Comments.post('/deletecomment/:videoId/:commentId/:email', async (req, res) => {
 try {
  ...
  const existingCommentIndex = comments.findIndex(comment => comment._id.toString() === commentId)
  ...
  if (existingCommentIndex !== -1) {
   ...
   res.status(200).json({message: 'Comment Deleted', commentData: comments})
  } else {
   return res.status(404).json({error: 'Comment not found'})
  }
 } catch (error) {
