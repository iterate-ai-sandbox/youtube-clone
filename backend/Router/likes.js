...
Likes.post('/like/:id/:email/:email2', async (req, res) => {
 try {
  ...
  const existingLikedVideo = user.likedVideos.find(likedVideo => likedVideo.likedVideoID === likedData._id.toString())
  ...
  if (!existingLikedVideo) {
   ...
   res.status(200).json({message: 'Liked', likes: video?.VideoData[videoIndex]?.likes})
  } else {
   user.likedVideos = user.likedVideos.filter(likedVideo => likedVideo.likedVideoID !== likedData._id.toString())
   video.VideoData[videoIndex].likes -= 1
   res.status(200).json({
    message: 'Disliked',
