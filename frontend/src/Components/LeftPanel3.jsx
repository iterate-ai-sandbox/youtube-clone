...
function LeftPanel2() {
 ...
 useEffect(() => {
  localStorage.setItem('studioMenuClicked2', JSON.stringify(studioMenuClicked))
 }, [studioMenuClicked])
 ...
 return (
  <>
   <div className={theme ? 'main-section3' : 'main-section3 light-mode text-light-mode'} style={studioMenuClicked === true ? {display: 'none'} : {display: 'flex'}}>
    <div
     className={theme ? 'first-panel first-panel1' : 'first-panel first-panel1 preview-lightt'}
     onClick={() => {
    ...
    <SkeletonTheme baseColor={theme ? '#353535' : '#aaaaaa'} highlightColor={theme ? '#444' : '#b6b6b6'}>
     <div className="mid-panel" style={loading === true ? {display: 'block'} : {display: 'none'}}>
    ...
    {/* END HERE  */}
    <div className="mid-panel" style={videodata && loading === false ? {visibility: 'visible', display: 'block'} : {visibility: 'hidden', display: 'none'}}>
     <div
     ...
     >
      ...
      <YouTubeIcon className="watch-video" fontSize="large" style={{color: 'white'}} />
     </div>
     <div className="thisvideo-mg-data">
      <p className="ur-vid">Your video</p>

      <p className={theme ? 'current-video-title' : 'current-video-title text-light-mode2'}>{videodata && videodata?.Title?.length <= 38 ? videodata && videodata?.Title : `${videodata && videodata?.Title?.slice(0, 38)}...`}</p>
     </div>
    </div>
    <div className="second-panel">
     <div
      className={VideoEditSection === 'Details' ? `${theme ? 'studio-active' : 'studio-active-light'} panel ${theme ? '' : 'panel-light'}` : `details panel ${theme ? '' : 'panel-light'}`}
      onClick={() => {
     ...
     <div
      className={VideoEditSection === 'Video-Comments' ? `${theme ? 'studio-active' : 'studio-active-light'} panel ${theme ? '' : 'panel-light'}` : `comments panel ${theme ? '' : 'panel-light'}`}
      onClick={() => {
   ...
   <div className={theme ? 'main-section3' : 'main-section3 light-mode'} style={studioMenuClicked === false ? {display: 'none'} : {display: 'flex', width: '90px'}}>
    <div
     className={theme ? 'first-panel first-panel1' : 'first-panel first-panel1 preview-lightt'}
     onClick={() => {
      if (window.location.href.includes(`/studio/video/edit/${id}`)) {
   ...
   <div className={theme ? 'main-section3-new' : 'main-section3-new light-mode'}>
    <div
     className={theme ? 'first-panel first-panel1' : 'first-panel first-panel1 preview-lightt'}
     onClick={() => {
      if (window.location.href.includes(`/studio/video/edit/${id}`)) {
