...
function VideoSection() {
 ...
 useEffect(() => { const getVideoData = async () => { try { if (id) { const response = await fetch(`${backendURL}/videodata/${id}`); const video = await response.json(); setVideoData(video); } } catch (error) { //console.log(error.message); } }; getVideoData(); }, [id]); useEffect(() => { if (Title && videoLength) { mixpanel.track('video page opened', { 'video title': Title, 'video length': videoLength }); } }, [Title, videoLength]);
 useEffect(() => {
       ...
       <div
        ...
        onClick={() => {
         if (shareClicked === false) {
          ...
          <div className="share" onClick={() => { mixpanel.track('video share initiated', { 'video title': Title }); setShareClicked(true); document.body.classList.add('bg-css'); }}>
         } else {
          setShareClicked(false)
          document.body.classList.remove('bg-css')
         }
