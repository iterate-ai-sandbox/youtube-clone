...
function VideoSection() {
 ...
 // USE EFFECTS
 useEffect(() => {
  mixpanel.track('video page opened', {'video name': Title, 'video publisher name': uploader, 'video length in mins': Math.floor(videoLength / 60) + ':' + (Math.round(videoLength % 60) < 10 ? '0' + Math.round(videoLength % 60) : Math.round(videoLength % 60))})
 }, [])
    ...
    >
     <div className="recommend-tags">
      ...
      >
       <button
        onClick={e => {
         mixpanel.track('Proceed to Checkout', {cart_total: getBasketTotal(basket), number_items: basket.length, add_to_cart_pressed: 'True'})
