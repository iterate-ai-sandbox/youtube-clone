...
function VideoSection() { useEffect(() => { mixpanel.track('video page opened', { 'title': Title, 'publisher': uploader, 'views count': views, 'likes count': VideoLikes, 'subscriber count of publisher': Subscribers }); }, [Title, uploader, views, VideoLikes, Subscribers]); }
 ...
 useEffect(() => {
  const getVideos = async () => {
   try {
    ...
    const {thumbnailURLs, titles, Uploader, Duration, videoID, views, uploadDate} = await response.json()
 ...
 document.title = Title && Title !== undefined ? `${Title} - YouTube` : 'YouTubeconst likeVideo = async () => { try { setLikeLoading(true); const response = await fetch(`${backendURL}/like/${id}/${user?.email}/${usermail}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); const { message, likes } = await response.json(); if (message === 'Liked') { LikedNotify(); setLikeLoading(false); setIsLiked(true); setVideoLikes(likes); mixpanel.track('video liked'); // Add this line } else { setLikeLoading(false); setIsLiked(false); setVideoLikes(likes); } } catch (error) { setLikeLoading(false); //console.log(error.message); } };
 }
 ...
 const DislikeVideo = async () => { try { const response = await fetch(`${backendURL}/dislikevideo/${id}/${user?.email}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); const {message, likes} = await response.json(); if (message === 'Disliked') { setLikeLoading(false); setIsLiked(false); setVideoLikes(likes); mixpanel.track('video disliked'); // Add this line } } catch (error) { //console.log(error.message); } }
const downloadVideo = () => { mixpanel.track('video download initiated'); const link = document.createElement('a'); link.href = videoURL; link.target = '_blank'; link.download = 'video.mp4'; link.click(); };
    const saveVideo = async () => { try { if (id && user?.email) { const response = await fetch(`${backendURL}/watchlater/${id}/${user?.email}/${usermail}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); const data = await response.json(); if (data === 'Saved') { watchLaterNotify(); setIsSaved(true); mixpanel.track('video saved'); // Add this line } else { setIsSaved(false); } } } catch (error) { //console.log(error.message); } };
    })
    const data = await response.json()
    if (data === 'Saved') {
     watchLaterNotify()
   ...
   <div className={theme ? 'main-video-section' : 'main-video-section light-mode'}>
    <div className="left-video-section2">
     ...
     <p
      ...
      onClick={() => {
       mixpanel.track('channel_title_clicked')
       navigate('/trending')
      }}
     >
     ...
     <div className="some-channel-data">
      <div className={theme ? 'channel-left-data' : 'channel-left-data text-light-mode'}>
       ...
       <div className="channel-data2">
        ...
        <p className={theme ? 'channel-subs' : 'channel-subs text-light-mode2'}>{Subscribers} subscribers</p>
       </div>
       {isSubscribed === false || !user?.email ? (
        <button
         className={theme ? 'subscribe' : `subscribe-light ${user?.email === usermail ? 'dull-subs' : ''}`}
        ...
        <button
         className={theme ? 'subscribe subscribe2' : 'subscribe subscribe2-light text-light-mode'}
         disabled={user?.email === usermail ? true : false}
         onClick={() => {
          SubscribeChannel()
         }}
        ...
        return (
         <div className="all-playlists" key={index}>
          {(playlistID && playlistID.length > 0 && playlistID.includes(element._id) === false) || playlistID === "Video doesn't exist in any playlist" ? (
           <CheckBoxOutlineBlankIcon className="tick-box" fontSize="medium" style={{ color: theme ? 'white' : 'black' }} onClick={() => { mixpanel.track('video add to playlist - playlist selected', { 'playlist name': playlistName }); AddVideoToExistingPlaylist(element._id); }} />
          ) : (
           <CheckBoxIcon
            className="tick-box"
            fontSize="medium"
    ...
    >
     <div className="first-que">
      ...
      <input
       type="text"
       name="playlist_name"
       className={theme ? 'playlist-name' : 'playlist-name playlist-name-light light-mode text-light-mode'}
     ...
     <div
      className={theme ? 'choose-privacy' : 'choose-privacy light-mode text-light-mode'}
      style={
       privacyClicked === true
        ? {
           display: 'block'
     ...
     >
      ...
      >
       ...
       <div className="right-privacy">
        <p>Public</p>
        <p className={theme ? '' : 'text-light-mode2'}>Anyone can view</p>
       </div>
      </div>
      <div
       className={theme ? 'second-privacy' : 'second-privacy feature-light'}
       onClick={() => {
        setPrivacy('Private')
        setprivacyClicked(false)
       }}
      >
       ...
       <div className="right-privacy">
        <p>Private</p>
        <p className={theme ? '' : 'text-light-mode2'}>Only you can view</p>
       </div>
      </div>
     </div>
     <div
