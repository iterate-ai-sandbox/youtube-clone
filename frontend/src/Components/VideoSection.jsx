...
function VideoSection() {
 ...
 useEffect(() => {
  if (videoData && videoData.VideoData) {
   ...
   if (matchedVideo) {
    mixpanel.track('video page opened', {'video name': matchedVideo.Title, 'video publisher': matchedVideo.uploader, 'video length in mins': Math.floor(matchedVideo.videoLength / 60) + ':' + (Math.round(matchedVideo.videoLength % 60) < 10 ? '0' + Math.round(matchedVideo.videoLength % 60) : Math.round(matchedVideo.videoLength % 60))})
   }
  }
 }, [])
