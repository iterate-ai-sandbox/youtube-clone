        ...
        >
         {isLiked === true && user?.email ? (
          ...
          <ThumbUpAltOutlinedIcon
           ...
           className="like-icon"
          />
<p className="like-count">{VideoLikes}</p> </div> <div className={theme ? 'vl' : 'vl-light'}></div> <div className={theme ? 'dislike-data' : 'dislike-data dislike-data-light text-light-mode'} onClick={() => { if (user?.email) { DislikeVideo() } else { setisbtnClicked(true); document.body.classList.add('bg-css'); } }}> <ThumbDownOutlinedIcon fontSize='medium' /> <div style={{ color: theme ? 'white' : 'black' }} className='dislike-icon'></div> </div> <div className={theme ? 'share' : 'share share-light text-light-mode'} onClick={() => { mixpanel.track('video share initiated'); if (shareClicked === false) { setShareClicked(true); document.body.classList.add('bg-css'); } else { setShareClicked(false); document.body.classList.remove('bg-css'); } }}> <ReplyIcon fontSize='medium' style={{ color: theme ? 'white' : 'black', transform: 'rotateY(180deg)' }} className='sharee-icon' /> <p className='share-txt'>Share</p> </div> <div className={theme ? 'download-btn' : 'download-btn download-btn-light text-light-mode'} onClick={downloadVideo}> <h3> <LiaDownloadSolid fontSize={24} className='download-icon' color={theme ? 'white' : 'black'} /> </h3> <p className='download-txt'>Download</p> </div> <div className={theme ? 'save-later' : 'save-later save-later-light text-light-mode'} onClick={() => { if (user?.email) { saveVideo() } else { setisbtnClicked(true); document.body.classList.add('bg-css'); } }}> {isSaved === true ? ( <BookmarkAddedIcon fontSize='medium' style={{ color: theme ? 'white' : 'black' }} className='save-video-icon' /> ) : ( <BookmarkAddIcon fontSize='medium' style={{ color: theme ? 'white' : 'black' }} className='save-video-icon' /> )} <p>{isSaved === true ? 'Saved' : 'Save'}</p> </div>
