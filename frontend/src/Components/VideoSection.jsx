...
function VideoSection() {
 ...
 //           <ReactLoading
 //             type={"spin"}
// color={"white"} // height={50} // width={50} // /> // <p style={{ marginTop: "15px" }}> // Fetching the data, Hang tight...{" "} // </p> // </div> // </div> // </> // ); // } const {videoURL, Title, thumbnailURL, ChannelProfile, uploader, Description, views, videoLength, uploaded_date, visibility} = matchedVideo; document.title = Title && Title !== undefined ? `${Title} - YouTube` : 'YouTube'; const likeVideo = async () => { try { setLikeLoading(true); const response = await fetch(`${backendURL}/like/${id}/${user?.email}/${usermail}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); const { message, likes } = await response.json(); if (message === 'Liked') { LikedNotify(); setLikeLoading(false); setIsLiked(true); setVideoLikes(likes); mixpanel.track('video liked'); // Add this line } else { setLikeLoading(false); setIsLiked(false); setVideoLikes(likes); } } catch (error) { setLikeLoading(false); } }; const LikeComment = async commentId => { try { if (commentId !== undefined && id !== undefined && user?.email) { const response = await fetch(`${backendURL}/likecomment/${id}/${commentId}/${user?.email}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); let {message, likes} = await response.json(); if ((message = 'Comment liked successfully')) { setCommentLikes(likes); } else { setCommentLikes(likes) } } } catch (error) { //console.log(error.message); } }; const HeartComment = async commentID => { try { if (id !== undefined && channelID !== undefined) { const response = await fetch(`${backendURL}/heartcomment/${id}/${commentID}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); await response.json(); } } catch (error) { //console.log(error.message); } }; const DeleteComment = async commentId => { try { setCommentOpacity(0.34); const response = await fetch(`${backendURL}/deletecomment/${id}/${commentId}/${user?.email}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); const {message, commentData} = await response.json(); if (message === 'Comment Deleted') { setCommentOpacity(1); CommentDeleteNotify(); setComments(commentData) } // window.location.reload(); } catch (error) { //console.log(error.message); } } const DislikeVideo = async () => { try {    const response = await fetch(`${backendURL}/dislikevideo/${id}/${user?.email}`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
