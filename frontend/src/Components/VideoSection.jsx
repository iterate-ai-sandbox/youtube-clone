...
function VideoSection() { useEffect(() => { mixpanel.track('video page opened', { 'title': Title, 'publisher': uploader, 'views count': views, 'likes count': VideoLikes, 'subscriber count of publisher': Subscribers }); }, [Title, uploader, views, VideoLikes, Subscribers]);
 ...
 const saveVideo = async () => { try { if (id && user?.email) { const response = await fetch(`${backendURL}/watchlater/${id}/${user?.email}/${usermail}`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } }); const data = await response.json(); if (data === 'Saved') { watchLaterNotify(); setIsSaved(true); mixpanel.track('video saved'); // Add this line } else { setIsSaved(false); } } } catch (error) { // Handle error } };
    ...
    <div className="left-video-section2">
     ...
     <div className="some-channel-data">
      ...
      <div className="channel-right-data c-right1">
       ...
       <div className={theme ? 'download-btn' : 'download-btn download-btn-light text-light-mode'} onClick={() => { mixpanel.track('video download initiated'); downloadVideo(); }}
        <h3>
