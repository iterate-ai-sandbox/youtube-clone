            ...
            >
              {thumbnails &&
                ...
                thumbnails.map((element, index) => {
                  return (
                    <div className="video-data" key={index} style={ Visibility[index] === "Public" ? { display: "block", } : { display: "none", } } onClick={() => { mixpanel.track('video_clicked', { video_name: Titles[index] }); if (user?.success) { updateViews(VideoID[index]); setTimeout(() => { navigate(`/video/${VideoID[index]}`); }, 400); } navigate(`/video/${VideoID[index]}`); }}>
                      <img
                        style={{
                          width: "330px",
                          borderRadius: "10px",
