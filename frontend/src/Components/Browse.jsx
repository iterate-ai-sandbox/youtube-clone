...
function Browse() {
 ...
 useEffect(() => { window.scrollTo(0, 0); mixpanel.track('home page opened', {'categories shown': Tags.join(', ')}); }, []);
 useEffect(() => {
           ...
           <div className={theme === true ? 'channel-basic-data' : 'channel-basic-data text-light-mode'}>
            ...
            <div className="channel-text-data">
             ...
             <div className={theme ? 'view-time' : 'view-time text-light-mode2'}>
              <div className="video-data" key={index} style={ Visibility[index] === 'Public' ? { display: 'block' } : { display: 'none' } } onClick={() => { mixpanel.track('video clicked', { category: TagsSelected, title: Titles[index], publisher: uploader[index], 'view count': VideoViews[index], 'length in seconds': duration[index] }); if (user?.success) { updateViews(VideoID[index]) setTimeout(() => { navigate(`/video/${VideoID[index]}`) }, 400) } navigate(`/video/${VideoID[index]}`) }} >
              <p
