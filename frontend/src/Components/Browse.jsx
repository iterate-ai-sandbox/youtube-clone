                ...
                thumbnails.map((element, index) => {
                  return (
                    <div
                      ...
                      onClick={() => { mixpanel.track('video_clicked', { title: Titles[index], length_of_video: duration[index] }); if (user?.success) { updateViews(VideoID[index]); setTimeout(() => { navigate(`/video/${VideoID[index]}`); }, 400); } navigate(`/video/${VideoID[index]}`); }}
                    >
                      <img
                        style={{
                          width: "330px",
