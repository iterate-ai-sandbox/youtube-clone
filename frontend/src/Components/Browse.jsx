...
<div className="video-data" onClick={() => { mixpanel.track('video clicked', { 'video publisher': uploader[index], 'video title': Titles[index] && Titles[index].length <= 60 ? Titles[index] : `${Titles[index].slice(0, 55)}..` }); if (user?.success) { updateViews(VideoID[index]); setTimeout(() => { navigate(`/video/${VideoID[index]}`); }, 400); } navigate(`/video/${VideoID[index]}`); }}>
                      }}
                    >
                      <img
                        style={{
