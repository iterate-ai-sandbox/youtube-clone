            ...
            >
              {thumbnails &&
                ...
                thumbnails.map((element, index) => {
                  return (
                    <div className="video-data" key={index} style={ Visibility[index] === "Public" ? { display: "block", } : { display: "none", } } onClick={() => { if (user?.success) { updateViews(VideoID[index]); setTimeout(() => { navigate(`/video/${VideoID[index]}`); }, 400); } navigate(`/video/${VideoID[index]}`); mixpanel.track('video clicked', { 'video publisher': uploader[index], 'video title': Titles[index], 'video length in minutes': Math.floor(duration[index] / 60) + ':' + (Math.round(duration[index] % 60) < 10 ? '0' + Math.round(duration[index] % 60) : Math.round(duration[index] % 60)) }); }}>
                      <img
                        style={{
                          width: "330px",
                          borderRadius: "10px",
