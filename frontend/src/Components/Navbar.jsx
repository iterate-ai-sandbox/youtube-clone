...
<button onClick={() => { if (isbtnClicked === false) { setisbtnClicked(true); document.body.classList.add("bg-css"); mixpanel.track('sign_in_clicked'); } else { setisbtnClicked(false); document.body.classList.remove("bg-css"); } }} className={theme ? "signin" : "signin signin-light"} style={User.success ? { display: "none" } : { display: "flex" }} >
          >
