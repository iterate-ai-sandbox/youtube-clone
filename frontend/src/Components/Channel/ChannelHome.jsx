...
function ChannelHome(prop) {
 ...
 return (
  <>
    ...
    {myVideos.length > 0 && myVideos[0].visibility === 'Public' ? (
     ...
     >
      ...
      <div className="video-metadata user-ka-video">
       {window.innerWidth <= 1180 ? <p className="myvideo-title">{myVideos[0].Title.length <= 50 ? myVideos[0].Title : `${myVideos[0].Title.slice(0, 50)}...`}</p> : <p className="myvideo-title">{myVideos[0].Title}</p>}
       <div className={theme ? 'video-oneliner-data' : 'video-oneliner-data text-light-mode2'}>
        ...
        <div className="view-time2">
         <p className="myviews">{myVideos[0].views >= 1e9 ? `${(myVideos[0].views / 1e9).toFixed(1)}B` : myVideos[0].views >= 1e6 ? `${(myVideos[0].views / 1e6).toFixed(1)}M` : myVideos[0].views >= 1e3 ? `${(myVideos[0].views / 1e3).toFixed(1)}K` : myVideos[0].views} views</p>
       ...
       {window.innerWidth <= 1180 ? <p className={theme ? 'myvideo-description' : 'myvideo-description text-light-mode2'}>{myVideos[0].Description.length <= 120 ? myVideos[0].Description : `${myVideos[0].Description.slice(0, 120)}...`}</p> : <p className={theme ? 'myvideo-description' : 'myvideo-description text-light-mode2'}>{myVideos[0].Description.length <= 250 ? myVideos[0].Description : `${myVideos[0].Description.slice(0, 250)}...`}</p>}
      </div>
     </div>
    ) : (
     ''
      ...
      AllVideos.sort(sortByViews2).map((element, index) => {
       return (
        ...
        >
         ...
         <div className="video-metadata2">
          <p className={theme ? 'video-title2' : 'video-title2 text-light-mode'}>{element.Title.length <= 50 ? element.Title : `${element.Title.slice(0, 50)}...`}</p>
          <div className={theme ? 'views-and-time' : 'views-and-time text-light-mode2'}>
   ...
   <div
    className={theme ? 'thischannel-no-content' : 'thischannel-no-content text-light-mode'}
    style={
     myVideos && myVideos.message === "USER DOESN'T EXIST" && user?.email !== prop?.newmail
      ? {
         display: 'block'
        }
      : {
   ...
   <div
    className={theme ? 'thischannel-no-content2' : 'thischannel-no-content2 text-light-mode'}
    style={
     myVideos && myVideos.message === "USER DOESN'T EXIST" && user?.email === prop?.newmail
      ? {
         display: 'flex'
