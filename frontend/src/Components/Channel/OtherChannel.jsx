...
function OtherChannel() {
  ...
  useEffect(() => {
    if (Section === "Home" && coverIMG !== "No data") {
      ...
      setTop("2%");
    } else if (Section !== "Videos" && coverIMG === "No data" || Section !== "Home" && coverIMG === "No data") {
      setTop("2%");
    } else if (Section !== "Videos" && coverIMG !== "No data" || Section !== "Home" && coverIMG !== "No data") {
      setTop("31%");
    }
  ...
  useEffect(() => {
    const checkSubscription = async () => {
      try {
        if (user?.email) {
          const response = await fetch(`${backendURL}/checksubscription/${id}/${user?.email}`);
          const {
            message
          } = await response.json();
          if (message === true) {
  ...
  const SubscribeChannel = async () => {
    try {
      ...
      const response = await fetch(`${backendURL}/subscribe/${id}/${user?.email}/${Email}`, {
        method: "POST",
        credentials: "include",
  ...
  return <>
      <Navbar />
      <LeftPanel />
      {coverIMG !== "No data" ? <div className="channel-cover">
          <img src={coverIMG} alt="Banner" loading="lazy" className="channel-cover-img" />
        </div> : ""}
      {ChannelProfile ? <div className={Top === "2%" ? "channel-main-content-nocover" : "channel-main-content"} style={{
      top: Top
    }}>
          <SkeletonTheme baseColor={theme ? "#353535" : "#aaaaaa"} highlightColor={theme ? "#444" : "#b6b6b6"}>
            <div className="channel-top-content" style={loading === true ? {
          display: "flex"
        } : {
          display: "none"
        }}>
                ...
                <div className="channel-topleft-data">
                  <div className="channel-left">
                    ...
                    <div className="channel-extra">
                      <Skeleton count={1} width={220} height={15} style={{
                    borderRadius: "4px"
                  }} className="sk-channel-liner" />
                    </div>
                    <div className="more-about">
                      <Skeleton count={1} width={200} height={14} style={{
                    borderRadius: "4px"
                  }} className="sk-channel-more" />
                    </div>
                  </div>
                  {user?.email === Email ? <div className="channel-right-content channel-dualbtns">
                      <Skeleton count={1} width={160} height={38} style={{
                  borderRadius: "20px"
                  ...
                  left: "25px"
                    ...
                    </div> : <div className="channel-right-content">
                      <Skeleton count={1} width={125} height={38} style={{
                  borderRadius: "20px"
        ...
        display: "flex"
            ...
            <div className={theme ? "channel-left-content" : "channel-left-content text-light-mode"}>
              ...
              <div className="channel-topleft-data">
                <div className="channel-left">
                  ...
                  <div className={theme ? "channel-extra" : "channel-extra text-light-mode2"}>
                    <p className="channeluser">@{username && username}</p>
                    <p className="my-subs">
                      {Subscribers && Subscribers} subscribers
                    </p>
                    {myVideos && myVideos.message !== "USER DOESN'T EXIST" ? <p className="my-videoscount">
                        {myVideos && myVideos.length} videos
                      </p> : <p className="my-videoscount">0 videos</p>}
                  </div>
                ...
                {user?.email === Email ? <div className="channel-right-content channel-dualbtns">
                    <button className={theme ? "customize-channel" : "customize-channel btn-light-mode"} onClick={() => {
                navigate("/studio/customize");
              }}>
                      Customize channel
                    </button>
                    <button className={theme ? "manage-videos" : "manage-videos btn-light-mode"} onClick={() => {
          ...
          <div className="channel-mid-content">
            ...
            reloadPage()
              ...
              {Section === "Videos" ? <p className={theme ? "channel-videos1" : "channel-videos2"} style={myVideos && myVideos.message === "USER DOESN'T EXIST" ? {
            display: "none"
          } : {
            display: "block"
          }} onClick={() => {
          ...
          {Section === "Home" || Section === "" ? <ChannelHome newmail={Email} /> : ""}
          {Section === "Videos" && myVideos && myVideos.message !== "USER DOESN'T EXIST" ? <ChannelVideos newmail={Email} /> : ""}
        ...
        </div> : <div className="main-trending-section">
