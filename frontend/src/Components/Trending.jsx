...
function Trending() {
  ...
  useEffect(() => {
    const getTrending = async () => {
      try {
        const response = await fetch("https://youtube-iterate-ai.vercel.app/gettrending");
  ...
  return <>
      <Navbar />
      <LeftPanel />
      {trendingVideos.length > 0 ? <div className="main-trending-section2">
          ...
          <div className="trending-videos-section">
            {trendingVideos.length > 0 && trendingVideos.map((element, index) => {
          return <>
                    <SkeletonTheme baseColor={theme ? "#353535" : "#aaaaaa"} highlightColor={theme ? "#444" : "#b6b6b6"}>
                      <div className="trending-video-data sk-trending-data" style={loading === true ? {
                display: "flex"
              } : {
                display: "none"
              }}>
                  ...
                  borderRadius: "12px"
                    ...
                    top: "10px"
                          ...
                          <Skeleton count={1} width={400} height={25} style={{
                    position: "relative",
                    left: "30px",
                    top: "15px"
                    ...
                    <div className="trending-video-data" key={index} onClick={() => {
              if (user?.email) {
              ...
              display: "none"
                      ...
                      <div className="trending-video-texts">
                        <p className={theme ? "trending-batch" : "trending-batch-light"}>
                          TRENDING #{index + 1}
                        </p>
                        <p className={theme ? "trending-title" : "trending-title text-light-mode"}>
                          {element.Title}
                        </p>
                        <div className={theme ? "trending-oneliner" : "trending-oneliner text-light-mode2"}>
                        ...
                        <p className={theme ? "trending-desc" : "trending-desc text-light-mode2"}>
                          {element.Description.length <= 140 ? element.Description : `${element.Description.slice(0, 80)}...`}
                        </p>
                      </div>
                    </div>
                  </>;
