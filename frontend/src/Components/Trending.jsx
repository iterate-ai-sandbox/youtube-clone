...
function Trending() {
 ...
 useEffect(() => {
  mixpanel.track('trending page opened', {
   'list of video titles': trendingVideos.map(video => video.Title),
   'count of trending videos': trendingVideos.length
  })
 }, [trendingVideos])
 //Update Views
         ...
         <>
          ...
          >
           ...
           <div className="trending-video-texts">
            <p className={theme ? 'trending-batch' : 'trending-batch-light'}>TRENDING #{index + 1}</p>
            <p className={theme ? 'trending-title' : 'trending-title text-light-mode'}>{element.Title}</p>
            <div className={theme ? 'trending-oneliner' : 'trending-oneliner text-light-mode2'}>
             ...
             <div className="trend-channel-extras">
              <p className="t-views"> {element.views >= 1e9 ? `${(element.views / 1e9).toFixed(1)}B` : element.views >= 1e6 ? `${(element.views / 1e6).toFixed(1)}M` : element.views >= 1e3 ? `${(element.views / 1e3).toFixed(1)}K` : element.views} views</p>
            ...
            <p className={theme ? 'trending-desc' : 'trending-desc text-light-mode2'}>{element.Description.length <= 140 ? element.Description : `${element.Description.slice(0, 80)}...`}</p>
           </div>
          </div>
         </>
        )
