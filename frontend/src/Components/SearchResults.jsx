      ...
      <div className="searched-channels-section">
       {searchedChannelData &&
        ...
        searchedChannelData.map((element, index) => {
         return (
          <div className={theme ? 'search-channel' : 'search-channel text-light-mode'} key={index}>
           <Skeleton
            count={1}
            width={130}
            height={130}
         ...
         <div className={theme ? 'search-channel' : 'search-channel text-light-mode'} key={index}>
          ...
          <div className="channel-flex-data">
           <div className="channel-extra-content" onClick={() => navigate(`/channel/${element._id}`)}>
            ...
            <div className="channel-liner">
             <p className={theme ? 'new-email' : 'new-email text-light-mode2'}>{userEmail && '@' + userEmail.split('@')[0]}</p>
             <p className={theme ? 'new-subs' : 'new-subs text-light-mode2'}>{element.subscribers} subscribers</p>
            </div>
            <p className={theme ? 'new-desc search-desc' : 'new-desc search-desc text-light-mode2'}> {element.channelDescription.length <= 100 ? element.channelDescription : `${element.channelDescription.slice(0, 100)}...`}</p>
           </div>
           <div className="subscribe-btnss">
            {user?.email === userEmail ? (
             ...
             <>
              <button
               className={theme ? 'subscribethis-channel' : 'subscribethis-channel-light text-dark-mode'}
               style={
                isSubscribed === true
                 ? {
                    display: 'none'
              ...
              <button
               className={theme ? 'subscribethis-channel2' : 'subscribethis-channel2-light'}
               style={
                isSubscribed === true
                 ? {
                    display: 'block'
         ...
         <>
          ...
          >
           ...
           <div className="thischannel-video-data">
            <p className={theme ? 'thisvideo-title' : 'thisvideo-title text-light-mode'}>{window.innerWidth <= 1200 ? <p>{element.Title.length <= 50 ? element.Title : `${element.Title.slice(0, 50)}..`}</p> : <p>{element.Title}</p>}</p>
            <div className={theme ? 'thisvideo-onliner' : 'thisvideo-onliner text-light-mode2'}>
             <p className="thisvideo-views">{element.views >= 1e9 ? `${(element.views / 1e9).toFixed(1)}B` : element.views >= 1e6 ? `${(element.views / 1e6).toFixed(1)}M` : element.views >= 1e3 ? `${(element.views / 1e3).toFixed(1)}K` : element.views} views</p>
            ...
            {window.innerWidth <= 970 ? <p className={theme ? 'thisvideo-desc' : 'thisvideo-desc text-light-mode2'}>{element.Description.length <= 50 ? element.Description : `${element.Description.slice(0, 50)}...`}</p> : <p className={theme ? 'thisvideo-desc' : 'thisvideo-desc text-light-mode2'}>{element.Description.length <= 120 ? element.Description : `${element.Description.slice(0, 120)}...`}</p>}
           </div>
