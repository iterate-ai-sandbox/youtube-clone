...
function Basic() {
 ...
 const SaveLinksData = async () => {
  try {
   ...
   if (user?.email) {
    ...
    const response = await fetch(`${backendURL}/savelinksdata/${user?.email}`, {
     method: 'POST',
     credentials: 'include',
 ...
 const saveData = async () => {
  try {
   setLoading(true)
   if (user?.email && channelName !== undefined && channelDescription !== undefined && channelID !== undefined) {
    const data = {
     channelName,
     channelDescription,
     channelID
    }
    const response = await fetch(`${backendURL}/updatechanneldata/${user?.email}`, {
     method: 'POST',
     credentials: 'include',
     body: JSON.stringify(data),
     headers: {
  ...
  <>
   ...
   >
    <div className="basic-name-section">
     <p className={theme ? 'basic-name-head' : 'basic-name-head text-light-mode'}>Name</p>
     <p className={theme ? 'basic-name-desc' : 'basic-name-desc text-light-mode2'}>Choose a channel name that represents you and your content. Changes made to your name and picture are visible only on YouTube.</p>
     {channelName === undefined ? (
      <input type="text" className={theme ? 'channel-name-inp' : 'channel-name-inp text-light-mode'} value="Loading..." style={{pointerEvents: 'none', cursor: 'wait'}} placeholder="Enter channel name" />
     ) : (
      <input
       type="text"
    ...
    <div className="basic-description-section">
     ...
     {channelDescription === undefined ? (
      <textarea name="channel-desc" className={theme ? 'basic-channel-desc' : 'basic-channel-desc text-light-mode'} style={{pointerEvents: 'none', cursor: 'wait'}} value="Loading..." cols="30" rows="10" maxLength={1000}></textarea>
     ) : (
      <textarea
       name="channel-desc"
    ...
    <div className="basic-channelurl-section">
     ...
     <p className={theme ? 'basic-channelurl-desc' : 'basic-channelurl-desc text-light-mode2'}>This is the standard web address for your channel. It includes your unique channel ID, which is the numbers and letters at the end of the URL.</p>
     {channelID === '' || channelID === 'undefined' ? (
      <input type="text" className={theme ? 'channel-name-inp2' : 'channel-name-inp2 text-light-mode preview-light'} value="Loading..." style={{pointerEvents: 'none'}} />
     ) : (
      <>
       <div className="channellink-copy">
        <input type="text" className={theme ? 'channel-name-inp2' : 'channel-name-inp2 text-light-mode preview-light'} value={`${channelUrl}/${copy ? (channelID.length >= 30 ? channelID : `${channelID.slice(0, 10)}...`) : channelID}`} onClick={handleCopyLink} ref={channelIDInputRef} />
        <ContentCopyOutlinedIcon className={theme ? 'coppy-id' : 'coppy-id-light'} onClick={handleCopyLink} fontSize="medium" style={{color: theme ? 'white' : '#606060'}} />
       </div>
      </>
     )}
    </div>
    <div className="basic-links-section">
     ...
     <p className={theme ? 'basic-link-desc' : 'basic-link-desc text-light-mode2'}>Share external links with your viewers. They&apos;ll be visible on your channel&apos;s about page. Note: To delete/remove the pre-added link kindly leave the input field empty.</p>
     {Links &&
      Links.length > 0 &&
        ...
        <div className="uerlinks-data" key={index}>
         {element.instagram && element.instagram !== '' && (
          <div className="users-instalink">
           <InstagramIcon fontSize="large" className={theme ? 'studio-social-icons' : 'studio-social-icons social-lightt'} style={{color: theme ? 'white' : '#606060'}} />
           <input
            type="text"
         ...
         {element.facebook && element.facebook !== '' && (
          <div className="users-fblink">
           <FacebookIcon fontSize="large" className={theme ? 'studio-social-icons' : 'studio-social-icons social-lightt'} style={{color: theme ? 'white' : '#606060'}} />
           <input
            type="text"
         ...
         {element.twitter && element.twitter !== '' && (
          <div className="users-twitterlink">
           <TwitterIcon fontSize="large" className={theme ? 'studio-social-icons' : 'studio-social-icons social-lightt'} style={{color: theme ? 'white' : '#606060'}} />
           <input
            type="text"
         ...
         {element.website && element.website !== '' && (
          <div className="users-weblink">
           <LanguageIcon fontSize="large" className={theme ? 'studio-social-icons' : 'studio-social-icons social-lightt'} style={{color: theme ? 'white' : '#606060'}} />
           <input
            type="text"
         ...
         <div className="add-newlink-social" onClick={() => setAddLink(!addLink)}>
          <AddIcon fontSize="medium" style={{color: '#3eaffe'}} />
          <p>ADD LINK</p>
         </div>
         <div className={theme ? 'add-more-socials' : 'add-more-socials add-social-light'} style={addLink ? {display: 'flex'} : {display: 'none'}}>
          {!element.instagram && (
           <InstagramIcon
            fontSize="large"
            className={theme ? 'addthis-icon' : 'addthis-icon-light'}
