  ...
  try {
   ...
   return new Promise((resolve, reject) => {
    uploadData.on(
     ...
     async () => {
      try {
       const downloadURL1 = await getDownloadURL(uploadData.snapshot.ref)
  ...
  <>
   ...
   >
    <div className="profile-update-section">
     <p className={theme ? 'profile-head-txt' : 'profile-head-txt text-light-mode'}>Picture</p>
     <p className={theme ? 'profile-desc-txt' : 'profile-desc-txt text-light-mode2'}>Your profile picture will appear where your channel is presented on YouTube, like next to your videos and comments.</p>
     <p className={theme ? 'profile-desc-txt' : 'profile-desc-txt text-light-mode2'}>(Please refresh the page if the images don’t load properly.)</p>
     <div className="picture-section">
      <SkeletonTheme baseColor={theme ? '#353535' : '#aaaaaa'} highlightColor={theme ? '#444' : '#b6b6b6'}>
       <div className="pic-div" style={fakeLoading === true ? {display: 'flex'} : {display: 'none'}}>
        <Skeleton
         count={1}
         width={140}
         height={140}
      ...
      <div className="pic-div" style={fakeLoading === false ? {visibility: 'visible', display: 'flex'} : {visibility: 'hidden', display: 'none'}}>
       <img src={previewProfile} alt="profile" className="channel-image" />
      </div>
      <div className={theme ? 'pic-extra-content' : 'pic-extra-content text-light-mode2'}>
       ...
       <label className={theme ? 'change-image' : 'change-image blue-txt'} htmlFor="profile-image-input">
        CHANGE
       </label>
       <input type="file" id="profile-image-input" accept="image/*" onChange={handleProfileChange} style={{display: 'none'}} />
      </div>
     </div>
    </div>
    <div className="cover-update-section">
     <p className={theme ? 'cover-head' : 'cover-head text-light-mode'}>Banner image</p>
     <p className={theme ? 'banner-desc' : 'banner-desc text-light-mode2'}>This image will appear across the top of your channel</p>
     <p className={theme ? 'profile-desc-txt' : 'profile-desc-txt text-light-mode2'}>(Please refresh the page if the images don’t load properly.)</p>
     <div className="banner-section">
      ...
      <div className="pic-div" style={fakeLoading === false ? {visibility: 'visible', display: 'flex'} : {visibility: 'hidden', display: 'none'}}>
       {previewBanner ? <img src={previewBanner} alt="banner" className="banner-image" /> : ''}
      </div>
      <div className={theme ? 'pic-extra-content' : 'pic-extra-content text-light-mode2'}>
       For the best results on all devices, use an image that’s at least 2048 x 1152 pixels and 6MB or less.
       <label className={theme ? 'change-image' : 'change-image blue-txt'} htmlFor="banner-image-input">
        CHANGE
       </label>
       <input type="file" id="banner-image-input" accept="image/*" onChange={handleBannerChange} style={{display: 'none'}} />
      </div>
     </div>
    </div>
   </div>
