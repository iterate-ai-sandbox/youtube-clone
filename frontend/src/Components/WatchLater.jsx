...
function WatchLater() {
 ...
 const {user} = User
 document.title = 'Watch later - YouTubeupdated code useEffect(() => { setTimeout(() => { setLoading(false) }, 1000); mixpanel.track('watch later page opened', { 'count of videos to watch later': watchlater.length }); }, [])
 useEffect(() => {
       ...
       >
        {watchlater.length > 0
         ? watchlater.map((element, index) => {
            return (
             <div
              className={theme ? 'liked-all-videos' : 'liked-all-videos liked-all-videos-light'}
              key={index}
              style={{
