...
function replaceWindowLocationWithNavigate(ast) {
 traverse(ast, {
  AssignmentExpression(path) {
   if (t.isMemberExpression(path.node.left) && t.isIdentifier(path.node.left.object, {name: 'window'}) && t.isIdentifier(path.node.left.property, {name: 'location'}) && t.isMemberExpression(path.node.left.property) && t.isIdentifier(path.node.left.property.property, {name: 'href'})) {
    ...
    if (t.isStringLiteral(argument) || t.isTemplateLiteral(argument)) {
     path.replaceWith(t.callExpression(t.identifier('navigate'), [argument]))
    }
   }
  },
  MemberExpression(path) {
   if (t.isIdentifier(path.node.object, {name: 'window'}) && t.isIdentifier(path.node.property, {name: 'location'})) {
    ...
    if (parent.isMemberExpression() && t.isIdentifier(parent.node.property, {name: 'href'})) {
     ...
     if (grandParent.isAssignmentExpression()) {
      ...
      if (t.isStringLiteral(argument) || t.isTemplateLiteral(argument)) {
       grandParent.replaceWith(t.callExpression(t.identifier('navigate'), [argument]))
      }
     }
    }
   }
